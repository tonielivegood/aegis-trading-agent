...
signatures, so the ENTRY trigger differs by class:
  ...
  - MAJOR: a 3x-volume spike is rare, so enter on a smaller but still-elevated
    bump (≥2.5x volume) caught EARLY (≤+5%, before the move is spent).
  - MEME : a real ≥3x-volume breakout is the trigger, caught EARLY (≤+6%).
...
PARAMS: dict[str, ClassParams] = {
    # MAJOR: looser entry (≥2.5x, +0.3%..+5%) — but RIDE, no scalp.
    MAJOR: ClassParams(vol_mult=2.5, breakout_min=0.003, breakout_max=0.05,
